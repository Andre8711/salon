...
def experienced_hairdressers(request):
    masters = Master.objects.filter(specialization='hair', experience__gt=5)
...
def salons_with_cosmetologists(request):
    salons = Salon.objects.filter(masters__specialization='cosmetology').distinct()
    return render(request, 'salon/salons_with_cosmetologists.html', {'salons': salons})

def expensive_service_masters(request):
    masters = Master.objects.filter(master_services__service__price__gt=3000).distinct()
...
def professional_masters(request):
    masters = Master.objects.filter(
        experience__gt=3,
        rating__gt=4.0,
        salon__is_active=True
    )
    return render(request, 'salon/professional_masters.html', {'masters': masters})